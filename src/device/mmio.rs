...
pub trait VirtioMmioDevice<M: GuestAddressSpace>: VirtioDevice<M> {
    // This is part of the `VirtioMmioDevice` interface instead of `VirtioDevice` because it looks
    // like the interrupt status is handled differently between transports (for example, the
    // interrupt status appears to be clear-on-read for PCI devices).
    ...
    /// Handle a read operation from the driver over the MMIO space of the device.
        ...
        match offset {
            0x00..=0xff if data.len() == 4 => {
                let v = match offset {
                    ...
                    0x44 => self.queue().map(|q| q.ready).unwrap_or(false).into(),
    ...
    /// Handle a write operation from the driver over the MMIO space of the device.
