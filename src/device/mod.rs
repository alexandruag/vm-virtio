...
pub use virtio_config::{VirtioConfig, WithDeviceOps, WithVirtioConfig};
...
pub trait VirtioDevice<M: GuestAddressSpace> {
    ...
    /// Return the number of feature pages (32 bit wide) used by the device.
    fn num_feature_pages(&self) -> u32;

    /// Return the specified device features page.
    fn device_features(&self, page: u32) -> u32;

    /// Return the specified page of features written by the driver.
    ...
    fn ack_device_status(&mut self, status: u8) {
        ...
        match !current_status & status {
            ...
            FEATURES_OK if current_status == (ACKNOWLEDGE | DRIVER) => {
                // Iterate through all feature pages and ensure the driver did not ack
                // a feature that wasn't actually offered. If that happens, the standard
                // specifies the devices should not actually accept/set the `FEATURES_OK`
                // status bit.
                for page in 0..self.num_feature_pages() {
                    let device_features = self.device_features(page);
                    let driver_features = self.driver_features(page);
                    if !device_features & driver_features != 0 {
