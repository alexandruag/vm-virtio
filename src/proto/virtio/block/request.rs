...
use super::{BlockBackend, Error, SECTOR_SHIFT, SECTOR_SIZE};
...
impl Request {
    ...
    pub(crate) fn execute<B: BlockBackend>(
        &self,
        backend: &mut B,
        mem: &GuestMemoryMmap,
        ...
        match self.request_type {
            RequestType::In => {
                mem.read_from(self.data_addr, backend, self.data_len as usize)
            ...
            RequestType::Out => {
                mem.write_to(self.data_addr, backend, self.data_len as usize)
                    .map_err(ExecuteError::Write)?;
                // METRICS.block.write_bytes.add(self.data_len as usize);
                // METRICS.block.write_count.inc();
            }
            RequestType::Flush => match backend.flush() {
                Ok(_) => {
